...
| 2 | **대사** | 등장인물 | 만화 **말풍선 = 직사각형** (폭은 컷의 **20%** · 세로로 길게 · 주인공=왼쪽 위, 상대방=오른쪽 위) |
| 2 | **속마음** | 주인공 | 만화 **속마음 풍선 = 타원** (폭은 컷의 20% · 세로는 그 폭에 글자를 넣는 데 필요한 만큼) |
...
- **몸통은 글자보다必ず 크게** 그려집니다. 폭은 컷의 20% × 2.2(보스트), 최소 높이는 컷의 42%이고,
  자산의 **글자 안전여백은 알파에서 자동 계산**해 9슬라이스 사영 후 적용합니다.
  결과: 같은 문장 기준 벡터때보다 면적 2.2배, 글자 영역의 배경 노출 0.5% 이하.
