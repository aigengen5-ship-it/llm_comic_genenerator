...
| # | 화면 요소 | 발화 | 화면 그림 |
|---|---|---|---|
| 1 | **설명**(지문) | 작가 | 컷 **하단 왼쪽** 흰 박스 + 검은 테두리 + 검은 글씨 — **글자 수에 맞춰 작게** |
| 2 | **대사** | 등장인물 | 만화 **말풍선** (주인공=왼쪽 위, 상대방=오른쪽 위 · 꼬리는 아주 작게) |
...
| 설명 박스가 대화에 비해 여전히 크다 | 설명(`caption_ko`)이 길어서입니다 — 이벤트 컷은 최장 2줄로 줄어들므로 문장을 나눠 주세요 |
