...
| `--no-epilogue` | ★에필로그 페이지(반투명 이벤트신 2컷 + 큰 지문)를 붙이지 않습니다 |
| `--no-summary-cuts` | ★서두 요약 컷(기승전결 첫 컷 = 배경만 + 컷 70% 큰 지문)을 끕니다 |
...
| 단계 | 결정자 | 근거 |
|---|---|---|
| ① 본문 → 컷 수 | `comic_input.target_panels()` | `본문 글자 수 ÷ --chars-per-panel(600)`, 하한 6, 상한 `--max-panels`(0=무제한) |
...
| # | 화면 요소 | 발화 | 화면 그림 |
|---|---|---|---|
| 1 | **설명**(지문) | 작가 | 컷 **하단 왼쪽** 흰 박스 + 검은 테두리 + 검은 글씨 |
| 2 | **대사** | 등장인물 | 만화 **말풍선** (꼬리가 화자 쪽을 가리킴) |
| 2 | **속마음** | 주인공 | 만화 **속마음 풍선** (타원 + 작아지는 물방울) |
| 3 | **설명 + 대사** | 둘 다 | **큰 이벤트 컷** (회당 2~4컷만 권장) |

- 대사가 길면 **풍선 두 개**까지(한 풍선 한 마디). 화면에 화자 이름은 찍지 않고 풍선 꼬리만 방향을 가리킵니다.
- 의성어/의태어(`sfx`)는 대형 흰 글씨 + 검은 윤곽으로 살짝 기운 각도로 들어갑니다.
- 모든 요소는 컷 안에서만 쓰이고 서로 겹치지 않으며, 배치는 **결정론**입니다(같은 입력 → 같은 자리).
- 컷 스크립트(`episode_NN_script.json`)의 컷 필드: `caption_ko`(설명), `lines`(`{kind, who, text}` ≤2 · `kind`는 `speech`/`thought`), `sfx`(의성어).
  옛 `dialog: ["이름: 대사", …]` 출력도 그대로 받습니다(화자 자동 분리, `(...)`는 속마음으로 봅니다).

**★ 화면 문법이 강제되는 슬롯** — `data/cut.yaml`의 `tier.role`로 정하고, 코드는 role이 없어도 **각 페이지(=기승전결 막)의 첫 슬롯**을 자동으로 `summary`로 봅니다.

| 역할 | 어디에 | 화면 | 끄는 플래그 |
|---|---|---|---|
| **★서두 요약** | 각 기승전결의 가장 앞 컷 | 인물 없이 **배경만** + 상황 요약 지문이 컷 면적의 **70%** | `--no-summary-cuts` |
| **★에필로그** | 결 다음 1페이지 (전폭 이벤트신 2컷) | 컷을 **반투명**으로 처리하고 그 위에 여운의 큰 지문 | `--no-epilogue` |
...
| 대사가 너무 길어 잘린다 | 한 풍선은 `DIALOG_MAX_LEN`자까지입니다 — 긴 말은 두 개의 풍선으로 나누어 주세요(풍선은 최대 2개) |
| 에필로그가 안 붙는다 | `--no-epilogue`가 켜져 있거나 페이지 상한(`--max-pages`)에 닿았습니다 |
