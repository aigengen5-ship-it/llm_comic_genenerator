...
- **꼬리와 생각 물방울도 자산**이지만, 그대로 붙이지는 않습니다. 꼬리는 "이미지 크기"가
  아니라 **화자 좌표**에서 나오는 값이라 방향이 컷마다 연속적으로 바뀌기 때문입니다.
  그래서 자산은 **오른쪽을 가리키게 하나만** 그려 두고, 붙일 때 **길이·밑변·각도에 맞춰
  회전**시킵니다(필요하면 반전). 물방울은 원 자산 하나를 3번 축소해 화자 쪽으로 **몸통 바깥**에
  늘어놓습니다(예전은 몸통 안에 그려 같은 색으로 안 보였습니다).
...
| 꼬리 | `tail_plain` / `tail_sharp` / `tail_shout` / `tail_soft` | 몸통 변형에 대응, 붙일 때 회전 |
| 물방울 | `bubble` | 속마음 3연타(하나를 3번 축소) |
