...
| 2 | **대사** | 등장인물 | 만화 **말풍선 = 직사각형** (폭은 컷의 **15%** · 세로로 길게 · 컷당 **최대 3개** · 주인공=왼쪽, 상대방=오른쪽) |
| 2 | **속마음** | 주인공 | 만화 **속마음 풍선 = 타원** (폭은 컷의 15% · 세로는 그 폭에 글자를 넣는 데 필요한 만큼) |
...
- **풍선은 좁고 세로로 길게(2026-09-10: 20%→10%, 2026-09-11: 세로가 지나치게 길어져 **15%로 완화**)**: 말풍선이 얼굴을 덮지 않게 폭을 컷 폭의 비율로 고정하고, 대사를 그 폭에 맞춰 접어 세로로 늘립니다(글자 크기도 줄여서) · 속마음 타원의 세로는 '그 폭에 글자를 넣는 데 필요한 최소'만 씁니다(세로가 컷 높이의 36%를 넘으면 폭을 더 넓혀 줄 수를 줄입니다)
...
- 화면에 화자 이름은 찍지 않습니다. 풍선 자리(왼쪽/오른쪽)와 형태(직사각형/타원)로 화자를 가릅니다. 화자를 가리키던 꼬리(화살표)·생각 물방울(작은 원)은 정상 동작하지 않아 삭제했습니다.
...
- **꼬리(화살표)와 생각 물방울(작은 원)은 삭제**되었습니다(정상 동작하지 않아 사용자 지시로
  삭제). 풍선은 몸통만 붙이고, 화자는 자리(왼쪽/오른쪽)와 형태로 구분합니다.
- **몸통은 글자보다必ず 크게** 그려집니다. 폭은 컷의 15% × 2.2(보스트), 최소 높이는 컷의 42%이고,
...
| 원작 헤더 | 화면 |
|---|---|
| `[LOCATION]` + `[SITUATION]` + `[TIME]` | 세 헤더를 **그림 한 장**으로 합칩니다(배경 확립 컷 · 사람 없이 장소만) |
| `[CLOTHES]` | **전신 스탠딩 한 장.** 그 복장은 **다음 `[CLOTHES]`가 나올 때까지** 뒤따르는 모든 컷에 계속 할당됩니다 |
