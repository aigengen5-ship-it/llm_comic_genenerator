...
| `--partner-full` | 상대방을 최소 태그(`invisible man/woman`)가 아니라 시트의 상세 태그로 그립니다 (3-8g) |
...
| 2 | **대사** | 등장인물 | 만화 **말풍선 = 직사각형** (폭은 컷의 **30%** · 컷당 **최대 3개** · 주인공=왼쪽, 상대방=오른쪽) |
| 2 | **속마음** | 주인공 | 만화 **속마음 풍선 = 타원** (폭은 컷의 30% · 세로는 그 폭에 글자를 넣는 데 필요한 만큼) |
...
- **풍선 가로 폭은 컷의 30% (2026-09-10: 20%→10% · 2026-09-11: 15%로 완화 · 2026-09-14: 사용자 지시로 **30%로 확대**)**: 말풍선이 얼굴을 덮지 않게 폭을 컷 폭의 비율로 고정하고, 대사를 그 폭에 맞춰 접어 세로로 늘립니다(글자 크기도 줄여서) · 속마음 타원의 세로는 '그 폭에 글자를 넣는 데 필요한 최소'만 씁니다(세로가 컷 높이의 36%를 넘으면 폭을 더 넓혀 줄 수를 줄입니다)
...
### 3-8g) 상대방은 고정 그룹 하나로 — `--partner-full`
...
```
(bald featureless faceless naked nude <체형> invisible man:3.0)      ← 상대방 외모의 전부
```
