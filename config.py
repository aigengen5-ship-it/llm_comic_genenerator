...
# [스탠딩 재사용] 원작이 전신 한 장을 정한 컷([CLOTHES]/[STANDING])을 흰 배경 스프라이트 1장으로 뽑아
#   같은 장면의 배경 컷 위에 얹습니다(정본 anima_gen_standing과 같은 어구). 효과 확인 후 기본 ON 예정입니다.
comic_standing = False         # 면당 컷 수(실측 평균 ~5: 한 면 2~6컷) — 목표 면수의 환율
...
# 전폭(wide) 컷 비중 상한 — 도입부를 넓게 열어도 세로 컷이 회차에서 사라지지 않게 한다(1.0=제한 없음)
