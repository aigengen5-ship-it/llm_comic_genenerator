...
- STRICT SEPARATION: the source tags are split into [AAA ...] lines (protagonist only) and a [BBB] line (partner only). A tag from an [AAA ...] line may ONLY appear in the AAA section. The [BBB] line is a single fixed group - "(bald featureless faceless naked nude <body-type> invisible man:3.0)" (or "invisible woman") - and it is the partner's COMPLETE appearance: never expand, translate or invent the partner's hair/eyes/skin/face/beard/makeup/glasses/clothes anywhere in the output (including the observer section). Never copy a tag from one character to the other.
- BBB FIXED TAG GROUP (MANDATORY): output that group VERBATIM (same words, same order, same :3.0 weight) as the FIRST item of the observer section, and after it put ONLY the visible body parts and action phrases allowed by [OBSERVER]/[ACTION] (e.g. "his bare hands pressing her shoulder"). Adding or inventing ANY appearance tag for BBB is forbidden - no hair, no eye color, no skin tone ("tan hands" is an invented skin tone), no face, no beard, no glasses, no clothing, no body-shape words. The gender is already carried by the group ("invisible man"/"invisible woman"); keep the possessive gender wording ("his", "the man's") without adding appearance words.
...
- Observer section: only the observer's own body parts visible in a first-person frame, linked to the action, e.g. (BBB's hands gripping AAA's hips:1.6). Start the section with the fixed group above, then describe those visible parts with NOTHING but the action - the partner's appearance is closed. NEVER apply the protagonist's clothing, accessories (gloves, socks, jewelry) or body features (breast size/shape) to them. If [OBSERVER] says "not visible in the frame", write only that the observer is not visible (the fixed group is still the first item).
