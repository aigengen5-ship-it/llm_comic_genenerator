...
- **말풍선·속마음 형태**는 두 갈래: 기본은 코드 벡터, `--balloon-style image`는 `data/balloons/` 자산 9슬라이스 합성(`comic_page_merge.paste_balloon_art`). 감정→변형 선택은 `pick_balloon_variant`(같은 페이지 중복 회피, 결정론). 꼬리(화살표)·생각 물방울(작은 원)은 [2026-09-13]부터 **자산에 구워 넣는다** — 같은 몸통을 왼쪽 `_l`/오른쪽 `_r` 두 장 굽고(총 27장), 풍선이 놓이는 칸이 파일을 고른다(왼쪽 열 → `_l`). 합성 중에 새 도형을 그리지 않는다. `paste_balloon_art(tail=)`/`_bake_tail` — 꼬리는 **1:1.3 직각삼각형**, 짧은 변에는 검정 선을 안 그리고 그 변을 몸통 테투리에 붙인다(테투리가 끊기며 꼬리가 박스 밖으로 튀어나온다. 자산 아래에 `BALLOON_TAIL_ROOM=64` 확보). 플레이트는 [2026-09-14]부터 pure white(α=255) — 반투명 폐기. 자리는 9등분 중 4칸(왼쪽 위·왼쪽 중간·오른쪽 위·오른쪽 중간)만 쓰고, 주인공=왼쪽 열·상대방=오른쪽 열(POV도 오른쪽), 1개면 위칸부터. 한 컷 발화가 3개 이상이면 렌더 전에 두 컷으로 쪼갠다(`comic_gen._split_chatty_panels`, `DIALOG_PER_CUT=2`)하고 레이아웃을 다시 짠다. 몸통 폭은 컷 15%×2.2 보스트(2026-09-10 폭 절반 → 2026-09-11 15%로 완화)·최소 높이 42%, 글자 안전여백은 자산 알파에서 자동 계산해 9슬라이스 사영 후 적용. 자산 없으면 벡터 폴백.
...
- **성별이 프롬프트에 새지 않게**: 대명사는 주인공 성별(`config.sex`)을 따르고 **상대방이 같은 성별일 때만** 통째로 바꿉니다(`comic_gen.fix_pronoun_gender` — 혼합 커플의 `her`는 상대방 소유일 수 있어 보존). 수위 태그는 헤더가 단독 소유라 본문에서 제거합니다(`strip_safety_token`, 실측 `explicit` 2회). 남자 주인공의 하체는 팬티 착용 → `(bulge)`, 하의 없음 → `(futanari, glans)` + negative `(vagina)/(cameltoe)`(`male_lower_body` → `anima_gen.set_extra_negative`가 노드 87에 중복 없이 합친다). 남자 × 남자 삽입 컷은 자세를 두고 삽입만 항문으로 바꿉니다(`mm_anal_fix`: 'in her vagina'→'in his anus', `(anal:1.6)`, `(males_only:1.4)`, creampie→anal creampie) — 자세 시트가 여성 주인공 시점으로 쓰여 있어(실측 213회) 후처리가 교정합니다.
...
- 화면 문법: 설명 박스는 컷 **하단**, 폭 80%(대사와 공존 시 62%, ★도입 요약은 100%), 줄 제한 없음(글자가 많으면 **폰트가 먼저** 줄어듦, 하한 11px). 말풍선은 컷 폭 15% 직사각형, 속마음은 15% 타원(2026-09-10 폭 절반 → 2026-09-11 15%, 꼬리·물방울은 폐지). 컷당 풍선은 `BALLOON_MAX`(=3)까지이고 `comic_gen.DIALOG_LINES`가 같은 값을 따른다. 의성어·감정 표시(7종)는 컷 위/옆에 얹습니다.
