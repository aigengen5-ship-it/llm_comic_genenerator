...
- **말풍선·속마음 형태**는 두 갈래: 기본은 코드 벡터, `--balloon-style image`는 `data/balloons/` 자산 9슬라이스 합성(`comic_page_merge.paste_balloon_art`). 감정→변형 선택은 `pick_balloon_variant`(같은 페이지 중복 회피, 결정론). 꼬리(화살표)·생각 물방울(작은 원)은 정상 동작하지 않아 삭제됐다(몸통만 그린다). 몸통 폭은 컷 15%×2.2 보스트(2026-09-10 폭 절반 → 2026-09-11 15%로 완화)·최소 높이 42%, 글자 안전여백은 자산 알파에서 자동 계산해 9슬라이스 사영 후 적용. 자산 없으면 벡터 폴백.
