...
- **말풍선·속마음 형태**는 두 갈래: 기본은 코드 벡터, `--balloon-style image`는 `data/balloons/` 자산 9슬라이스 합성(`comic_page_merge.paste_balloon_art`). 감정→변형 선택은 `pick_balloon_variant`(같은 페이지 중복 회피, 결정론). 꼬리·생각 물방울은 두 갈래 모두 벡터로 그려 화자 조준을 유지한다. 자산 없으면 벡터 폴백.
