...
# 셸 환경변수 → 파이프라인이 읽는 COMIC_* 오버라이드로 번역 (plot.json을 건드리지 않는다)
if [ -n "${OLLAMA_BIN:-}" ]; then export OLLAMA_BIN; fi
if [ -n "${OLLAMA_MODELS_DIR:-}" ]; then export OLLAMA_MODELS_DIR; fi
if [ -n "${OLLAMA_HOST_URL:-}" ]; then export COMIC_OLLAMA_HOST="${OLLAMA_HOST_URL}"; fi

if [ "${1:-}" = "stop" ]; then
  shift
  exec python3 run_comic.py --stop-llm "$@"
fi
exec python3 run_comic.py --start-llm "$@"
